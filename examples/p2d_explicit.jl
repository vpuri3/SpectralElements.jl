...
u = cgSEM(bloc,M,QQt,B,Aloc);
nt = nx1*Ex*ny1*Ey;
opLapl=LinearOperator(nt,nt,true,true,v->Aloc*v,nothing,nothing)
...
#rr   = rand(size(AA,1));
#rloc = Q*Rg'*rr;
#bloc = ones(size(Aloc[:,1]));

#vv   = Q*Rg'*AA * rr  ; vv   = reshape(vv  ,nx1*Ex,ny1*Ey);
#vloc = Aloc     * rloc; vloc = reshape(vloc,nx1*Ex,ny1*Ey);
#
#nrm=norm(vv-vloc,Inf);
#print("vv-vloc: ",nrm,"\n");
#
#A2 = QRt * AA * QRt';
#n2 = norm(A2-Aloc,Inf)
